...
function truncate(s: string): string {
  const first = s.split("\n")[0]!;
  return first.length > MAX_INLINE
    ? `${first.slice(0, MAX_INLINE - 1)}…`
    : first;
}
...
export function KeyTable({
  ...
  const keyColWidth =
    Math.min(48, Math.max(16, ...keys.map((k) => k.key.length))) + 2;
    ...
    <Box flexDirection="column" flexGrow={1}>
      ...
      {keys.length === 0 ? (
        <Box paddingX={2} marginTop={1}>
          <Text dimColor>
            No keys found. Press <Text bold>a</Text> to add one.
          </Text>
        </Box>
      ) : (
        <>

...
import { scrollWindow, useTerminalRows } from "./useTerminalRows.js";
...
function buildDisplayMap(
  ...
  const decrypted = keys.some((k) => k.encrypted)
    ? decryptAllValues(filePath)
    : {};
  ...
  for (const k of keys) {
    if (k.encrypted) {
      ...
      out.set(
        k.key,
        plain !== undefined && !isEncryptedValue(plain) ? plain : "🔒",
      );
    } else {
...
export function DiffView({ left, files, onClose }: Props) {
  ...
  const chrome =
    9 + pickerVisible + (picker.above > 0 || picker.below > 0 ? 2 : 0);
  ...
  const keyColWidth =
    Math.min(48, Math.max(16, ...rows.map((r) => r.key.length))) + 2;
  ...
  return (
    <Box flexDirection="column">
      ...
      <Box paddingX={1}>
        ...
        <Text dimColor>
          diff {left.relativePath} ↔ {rightFile?.relativePath ?? "—"}
        </Text>
      </Box>
      ...
      <Box flexDirection="column" flexGrow={1} paddingX={1}>
        ...
        {visibleRows.map((row) => (
          <DiffRow key={row.key} row={row} keyColWidth={keyColWidth} />
        ))}
        {rowsBelow > 0 && <Text dimColor>↓ {rowsBelow} more</Text>}
        {rows.length === 0 && (
          <Box marginTop={1}>
            <Text dimColor>Select a file to compare.</Text>
          </Box>
        )}
      </Box>
